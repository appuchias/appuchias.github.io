...
Endlessh is a piece of software that sets up a ssh honeypot for all the random people that may try to attack you by trying common username and password combinations to see if they can get access. In return, this program slows their access down hu
gely, and in a way someone with a script to do the attack (probably what anyone sensible would do in this case), won't notice.
...
2. Compile the binary
   ...
   ```
3. Move the binary to a folder in the PATH (this is for ease of use, not mandatory but recommended)
   ```bash
   mv ./endlessh /usr/local/bin/
   # Make sure it is in the PATH by running `which endlessh`
   # It should return /usr/local/bin or the folder you set.
   ```
...
5. Create the config file
   ...
   ```bash
	...
	BindFamily 0
   ```
   **Important note**: if you're running this in a port<1024, you need to make some changes in `/etc/systemd/system/endlessh.service `: uncomment line `31` and comment line `33` (by adding # at the beginning).
...
### Attacker's POV
When someone tries to access your server this is what they will see:
```
ssh username@host
<forever blinking cursor>
```
The only way there is to know what is really happening is to add `-v` at the end of the command, which enables verbose mode and throws out a bunch more information about what's lagging the connection. Here you can see what is seen using `-v`:
```
