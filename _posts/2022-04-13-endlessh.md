...
![Github info about repo](/assets/images/2022-04-13-endlessh/skeeto_endlessh.png)
...
4. Copy the systemd unit file for endlessh
   ...
   **Important note**: If you're using Openrc instead of systemd, these are the commands you will need to run:

   ```bash
   # Copy the init file to the corresponding folder
   sudo cp util/smf/init.endlessh /etc/init.d
   # Allow execution
   sudo chmod +x /etc/init.d/init.endlessh
   # Enable the service on boot
   sudo rc-update add init.endlessh
   ```
